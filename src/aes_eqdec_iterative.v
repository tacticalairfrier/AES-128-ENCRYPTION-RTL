//this is for the equivalentinversecipher
`default_nettype none
`define TRUE 1'b1
`define FALSE 1'b0

module aes_eqdec_iterative(
...
//for all inclusive purposes keyexpansion here is keyexpansion eic
//comments for linter
...
begin
    ...
    temp_2 = temp;
    keyscheduler[127:96] = invmixcolumns(keyscheduler[127:96]);
    keyscheduler[95:64] = invmixcolumns(keyscheduler[95:64]);
    keyscheduler[63:32] = invmixcolumns(keyscheduler[63:32]);
    keyscheduler[31:0] = invmixcolumns(keyscheduler[31:0]);
    for(i=2;i<=9;i=i+1)begin
        //round 2 onwards
        temp[127:96] = shiftrows(temp[31:0]); //32 bit wide part select of keyschedule goes from 32 through 0
        temp[127:96] = subword(temp[127:96]);  //same logic
        temp[127:96] = temp[127:96] ^ {rcon(i), 24'h000000} ^ temp_2[127:96]; //key scheduler part select here to capture 127:96 here
        temp[95:64] = temp[127:96] ^ temp_2[95:64]; //keyscheduler here
        temp[63:32] = temp[95:64] ^ temp_2[63:32]; //ks here
        temp[31:0] = temp[63:32] ^ temp_2[31:0];//key in becomes hte keyscheduler variable
    ...
    temp[127:96] = shiftrows(temp[31:0]); //32 bit wide part select of keyschedule goes from 32 through 0
    temp[127:96] = subword(temp[127:96]);  //same logic
    temp[127:96] = temp[127:96] ^ {rcon(10), 24'h000000} ^ temp_2[127:96]; //key scheduler part select here to capture 127:96 here
    temp[95:64] = temp[127:96] ^ temp_2[95:64]; //keyscheduler here
    temp[63:32] = temp[95:64] ^ temp_2[63:32]; //ks here
    temp[31:0] = temp[63:32] ^ temp_2[31:0];//key in becomes hte keyscheduler variable
    //now keyscheduler can move on that is shifted by 128

...
`define FILE "vec/aes_test_vectors_dec.csv"
`define TRUE 1'b1
`define FALSE 1'b0

module test_eqdecrypt_exaustive;
//using 1000 test vectors list
//using ffscanf and fpopen
reg [799:0] line;
//800 bit buffer for storing each line
reg [127:0] key_in,ciphertext_in,out_expected;
//file inputs from csv
reg clkin, reset, enable;
//output wire variables
wire [127:0] out_tb;
wire busy;
aes_eqdec_iterative DUT_AES_EQDEC(
    .key(key_in),
    .cipher(ciphertext_in),
    .out(out_tb),
    .clkin(clkin),
    .reset(reset),
    .enable(enable),
...
initial begin
    $dumpfile("sim.vcd");
    $dumpvars(0,test_eqdecrypt_exaustive);
    //invoking reset
    reset = `FALSE;
    #10 reset = `TRUE;
    #10 enable = `TRUE;
    ...
    while(!$feof(j))begin
        $fgets(line, j);
        $sscanf(line, "%h, %h, %h", key_in, ciphertext_in, out_expected);
        enable = `TRUE;
        #130 enable = `FALSE;
        if(out_expected==out_tb)begin
            ...
            failed = failed+1;
            $display("%h, %h, for keys and ciphertexts -> for output %h expected was = %h", key_in, ciphertext_in, out_tb, out_expected);
        end
    end
